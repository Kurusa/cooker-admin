...
class XPathService
{
    ...
    {
        $nodes = $this->xpath
            ->query($query . ($isMetaAttribute ? '/@content' : ''));

        return array_map(fn(DOMElement|DOMAttr $node) => CleanText::cleanText($node->nodeValue), iterator_to_array($nodes));
    }

    public function extractSingleMetaAttribute(string $propertyName): string
    {
        return CleanText::cleanText(
            $this->xpath->query("//meta[@property='$propertyName']")
                ->item(0)
