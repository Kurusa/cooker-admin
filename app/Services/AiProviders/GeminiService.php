...
class GeminiService implements AiRecipeParserServiceInterface
{
    ...
    {
        $prompt = "На цій сторінці можуть бути один або кілька рецептів.Розпізнай кожен із них і поверни масив об’єктів у форматі JSON без жодних описів і пояснень.Кожен об’єкт має ключі:
-title(string)
-categories(array<string>):категорії без дублювання(це обов'язкове поле,тож визнач сам,якщо не вказано)
-complexity(string):завжди easy|medium|hard(визнач сам,якщо не вказано)
...
Якщо на сторінці один рецепт поверни масив із одним об’єктом. Ось HTML:" . $html;
        ...
        $response = $this->client->post('/v1beta/models/gemini-2.0-flash:generateContent?key=' . config('services.gemini.api_key'), [
            'json' => [
                'contents' => [
                    [
                        'parts' => [
        ...
        if (!$text || !preg_match('/```json\s*(\[.*\])\s*```/s', $text, $matches)) {
            throw new AiProviderDidntFindRecipeException('Gemini response invalid');
        }
